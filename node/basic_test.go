	...
	for round := 1; round <= testutil.BasicTestRoundNum; round++ {
		...
		for j := 1; j <= testutil.BasicTestRoundJoinNodeSize; j++ {
			...
			go func() {
				...
				munet.Lock()
				nodesInNetwork = append(nodesInNetwork, ii)
...
func TestMine(t *testing.T) {
	panicked, basicFailedCnt, basicTotalCnt := myTest()
	if panicked {
		t.Fatal("My test panicked")
	}
	if basicFailedCnt > 0 {
		t.Errorf("My test failed with fail num=%d/%d", basicFailedCnt, basicTotalCnt)
	}
}
